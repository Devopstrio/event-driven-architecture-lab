...
import { ShieldCheck, Activity, Database, TrendingUp, RefreshCcw, ExternalLink, Zap, Globe, AlertTriangle, Layers, Binary, Cpu, Wallet, Book, History, Lock, Fingerprint, Share2, Network, FlaskConical } from 'lucide-react';
...
const brokerDistribution = [
  { name: 'Kafka', value: 85, color: '#6366f1' },
  { name: 'RabbitMQ', value: 12, color: '#10b981' },
  { name: 'Event Hubs', value: 3, color: '#f59e0b' },
];
  ...
  return (
    <div className="space-y-12 animate-fade-in">
      ...
      <div className="flex justify-between items-end border-b border-slate-800 pb-8">
        <div>
          <h1 className="text-5xl font-black text-white tracking-tighter italic uppercase">Eventing Executive</h1>
          <p className="text-slate-400 mt-3 text-xl max-w-2xl font-medium">Institutional throughput analytics, messaging pattern governance, and real-time streaming maturity benchmarks.</p>
        </div>
        <div className="flex gap-4">
          <button className="bg-slate-900 border border-slate-800 text-slate-300 px-6 py-3 rounded-2xl font-bold hover:bg-slate-800 transition flex items-center gap-2">
             <RefreshCcw size={18} /> Sync Brokers
          </button>
          <button className="bg-indigo-600 hover:bg-indigo-500 text-white px-8 py-3 rounded-2xl font-bold transition shadow-xl shadow-indigo-900/40 flex items-center gap-2">
             <Zap size={18} /> Publish Test Event
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <StatCard title="Throughput (M/s)" value="12.5" change="+3.2" icon={<Activity className="text-indigo-400" />} />
        <StatCard title="Avg Latency" value="12ms" change="Stable" icon={<Zap className="text-emerald-400" />} />
        <StatCard title="Consumer Lag" value="1,450" change="-12%" icon={<AlertTriangle className="text-rose-400" />} />
        <StatCard title="Uptime SLA" value="99.99%" change="+0.01" icon={<ShieldCheck className="text-blue-400" />} />
      </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-10 rounded-[3rem] backdrop-blur-3xl shadow-2xl relative overflow-hidden">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              Institutional Global Event Throughput (Millions/Day)
            </h2>
            <div className="text-[10px] font-black uppercase text-indigo-400 tracking-[0.2em] bg-indigo-900/20 px-4 py-2 rounded-full border border-indigo-800/50">Streaming Engine</div>
          </div>
          <div className="h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={throughputTrend}>
                ...
                />
                <Area type="monotone" dataKey="events" stroke="#6366f1" strokeWidth={4} fillOpacity={1} fill="url(#colorEvents)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-slate-900/40 border border-slate-800 p-10 rounded-[3rem] backdrop-blur-3xl shadow-2xl relative overflow-hidden">
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3">
              ...
              Broker Distribution by Institutional Traffic Share
            </h2>
      ...
      <div className="bg-slate-900/40 border border-slate-800 p-10 rounded-[3rem] backdrop-blur-3xl shadow-2xl">
        <h2 className="text-2xl font-bold mb-10 text-white flex items-center gap-3 text-indigo-400 uppercase tracking-widest">
           ...
           Institutional EDA Simulation Labs
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
           <ProjectCard name="CQRS-Ecommerce" status="Active" score="99.9%" type="Pattern Lab" icon={<Network size={20} />} />
           <ProjectCard name="Saga-Payments" status="Running" score="98.4%" type="Reliability Lab" icon={<ShieldCheck size={20} />} />
           <ProjectCard name="IoT-Telemetry" status="Active" score="99.2%" type="Streaming Lab" icon={<Cpu size={20} />} />
        </div>
      </div>
    </div>
  );
...
const ProjectCard = ({ name, status, score, type, icon }: any) => (
  <div className="p-6 bg-slate-950/50 border border-slate-800 rounded-3xl hover:bg-slate-800/50 transition cursor-pointer group">
    <div className="flex justify-between items-start mb-4">
      ...
      <span className={`text-[10px] font-black px-3 py-1 rounded-full border border-slate-700 uppercase tracking-tighter ${status === 'Active' || status === 'Running' ? 'bg-emerald-950/20 text-emerald-400 border-emerald-800/50' : 'bg-indigo-950/20 text-indigo-400 border-indigo-800/50'}`}>{status}</span>
    </div>
